...
export function AlbumTab({
  ...
  const fetchPhotos = async () => {
    try {
      ...
      const {
        data: files,
        error
      } = await supabase.storage.from("event-media").list(prefix, {
        limit: 1000,
        sortBy: {
          column: "created_at",
          order: "desc"
      ...
      const photoData = files.filter((file: any) => {
        const ext = file.name.split('.').pop()?.toLowerCase();
        return ['jpg', 'jpeg', 'png', 'webp', 'gif'].includes(ext || '');
      }).map((file: any) => {
        const {
          data: pub
        } = supabase.storage.from("event-media").getPublicUrl(`${prefix}/${file.name}`);
        return {
          id: file.name,
          url: pub.publicUrl,
  ...
  const deletePhoto = async (photoId: string) => {
    ...
    try {
      ...
      const {
        error
      } = await supabase.storage.from("event-media").remove([`${prefix}/${photoId}`]);
      if (error) throw error;

      // Remove from local state immediately
      setPhotos(prev => prev.filter(p => p.id !== photoId));
      toast({
        title: "تم حذف الصورة نهائياً"
      });
  ...
  const handleSaveAlbumSettings = async () => {
    try {
      ...
      toast({
        title: "تم حفظ إعدادات الألبوم بنجاح"
      });
      onEventUpdate();
    } catch (error) {
      toast({
  ...
  const toggleAlbumPublish = async () => {
    ...
    try {
      const success = await updateEventSettings(token, {
        is_album_published: newStatus
      });
  ...
  return <div className="space-y-6" dir="rtl">
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            {albumCoverUrl && <div className="aspect-video w-full max-w-md rounded-lg overflow-hidden border">
          ...
          <div className="grid gap-4">
            <div className="space-y-2">
              ...
              <Input id="albumTitle" value={albumTitle} onChange={e => setAlbumTitle(e.target.value)} placeholder="عنوان الألبوم" />
            </div>

            <div className="space-y-2">
              <Label htmlFor="albumDescription">وصف الألبوم</Label>
              <Textarea id="albumDescription" value={albumDescription} onChange={e => setAlbumDescription(e.target.value)} placeholder="وصف قصير للألبوم" rows={3} />
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            {albumPublishTime === "manual" && <div className="flex items-center justify-between p-4 border rounded-lg">
                ...
                <Switch checked={isAlbumPublished} onCheckedChange={toggleAlbumPublish} />
              </div>}
          </div>
      ...
      <Card>
        ...
        <CardContent>
          {photos.length === 0 ? <div className="text-center py-8 text-muted-foreground">
              لا توجد صور في الألبوم حتى الآن
            </div> : <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {photos.map((photo, index) => <div key={photo.id} className="relative group">
                  <div className="aspect-square rounded-lg overflow-hidden border">
                    <img src={photo.url} alt={`صورة من ${photo.participant_name}`} className="w-full h-full object-cover cursor-pointer hover:scale-105 transition-transform" onClick={() => setLightboxIndex(index)} />
                  </div>
                  
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center gap-2">
                    <Button size="sm" variant="secondary" onClick={() => setLightboxIndex(index)}>
                      <Eye className="h-3 w-3" />
                    </Button>
                    <Button size="sm" variant="secondary" onClick={() => downloadPhoto(photo.url, `photo-${photo.id}.jpg`)}>
      ...
      {/* Lightbox */}
      {lightboxIndex !== null && <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <div className="relative max-w-4xl max-h-full">
            <img src={photos[lightboxIndex].url} alt="عرض الصورة" className="max-w-full max-h-full object-contain" />
            <Button variant="secondary" size="sm" className="absolute top-4 right-4" onClick={() => setLightboxIndex(null)}>
              إغلاق
            </Button>
          </div>
        </div>}
