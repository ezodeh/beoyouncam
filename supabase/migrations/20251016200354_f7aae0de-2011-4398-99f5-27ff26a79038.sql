...
-- Create trigger to prevent future plaintext passwords
  ...
  IF NOT EXISTS (
    ...
    WHERE token = event_token_param 
      ...
      AND (
        ...
        (password_hash IS NOT NULL AND password_hash = crypt(provided_password, password_hash))
        OR
        -- Fallback to plaintext during migration
        (password_hash IS NULL AND password = provided_password)
      )
  ) THEN
